...
function Notificacion({ n, onClose }: { n: Notif; onClose: () => void }) {
  ...
  return (
    <div
      className={`flex items-center justify-between gap-4 rounded-2xl border px-5 py-3 text-sm ${
        n.tipo === "ok"
    ...
    <main className="container-app py-10 space-y-8">
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 p-6 space-y-5">
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              >
                <option value="">
                  -- Selecciona {filterMode === "bodega" ? "bodega" : "presentación"} --
                </option>
                {filterMode === "bodega"
                  ? bodegas.map((b) => (
                  ...
                  : presentaciones.map((p, i) => renderPresentacionOption(p, i))}
              </select>
          ...
          {inventarioOrdenado.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                {
                  label: "Stock Disponible Total",
                  value: inventarioOrdenado.reduce((s, i) => s + i.stockDisponible, 0),
                  color: "text-emerald-600",
                },
                {
                  label: "Stock Reservado Total",
                  value: inventarioOrdenado.reduce((s, i) => s + i.stockReservado, 0),
                  color: "text-amber-600",
                },
                {
                  label: "Stock Bajo ⚠️",
                  value: stockBajoCount,
                  color: stockBajoCount > 0 ? "text-red-600" : "text-zinc-500",
                },
              ].map((stat) => (
                <div
                  key={stat.label}
                  className="rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 p-5"
                >
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-1">{stat.label}</p>
                  <p className={`text-2xl font-bold ${stat.color ?? "text-zinc-900 dark:text-white"}`}>
                    {stat.value}
                  </p>
                </div>
              ))}
                      ...
                      return (
                        <tr
                          ...
                          className={`hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition ${
                            item.stockBajo ? "bg-red-50/40 dark:bg-red-950/10" : ""
                          }`}
                        >
                          ...
                          <td className="px-5 py-3">
                            <select
                              value={op}
                              onChange={(e) =>
                                setOperaciones((prev) => ({ ...prev, [id]: e.target.value as Operacion }))
                              }
                          ...
                          <td className="px-4 py-3">
                            <input
                              ...
                              value={cantidades[id] ?? ""}
                              onChange={(e) =>
                                setCantidades((prev) => ({ ...prev, [id]: e.target.value }))
                              }
                          ...
                          <td className="px-4 py-3 text-center">
                            <button
                              onClick={() => ejecutarOperacion(item)}
                              disabled={loading}
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 p-6 max-w-2xl space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <select
                value={transfer.idPresentacion}
                onChange={(e) =>
                  setTransfer((p) => ({ ...p, idPresentacion: Number(e.target.value) }))
                }
            ...
            <div className="space-y-1">
              ...
              <select
                value={transfer.idBodegaOrigen}
                onChange={(e) =>
                  setTransfer((p) => ({ ...p, idBodegaOrigen: Number(e.target.value) }))
                }
                className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-4 py-2.5 text-sm"
              >
            ...
            <div className="space-y-1">
              ...
              <select
                value={transfer.idBodegaDestino}
                onChange={(e) =>
                  setTransfer((p) => ({ ...p, idBodegaDestino: Number(e.target.value) }))
                }
                className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-4 py-2.5 text-sm"
              >
                <option value={0}>-- Selecciona destino --</option>
                {bodegas
                  .filter((b) => b.idBodega !== transfer.idBodegaOrigen)
                  .map((b) => (
                    <option key={b.idBodega} value={b.idBodega}>
                      {b.nombre}
                    </option>
            ...
            <div className="sm:col-span-2 space-y-1">
              ...
              <input
                type="text"
                value={transfer.usuarioResponsable ?? ""}
      ...
      {tab === "crear" && (
        <div className="rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 p-6 max-w-xl space-y-6">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            Asocia una presentación de perfume a una bodega con su stock inicial.
          </p>
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <select
                value={createForm.idBodega}
                onChange={(e) => setCreateForm((p) => ({ ...p, idBodega: Number(e.target.value) }))}
                className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-4 py-2.5 text-sm"
              >
            ...
            <div className="space-y-1">
              ...
              <select
                value={createForm.idPresentacion}
                onChange={(e) =>
                  setCreateForm((p) => ({ ...p, idPresentacion: Number(e.target.value) }))
                }
                className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-4 py-2.5 text-sm"
              >
                <option value={0}>-- Selecciona presentación --</option>
              ...
              {[
                ...
                <div key={key} className="space-y-1">
                  ...
                  <input
                    type="number"
                    min="0"
                    value={(createForm as any)[key] ?? 0}
                    onChange={(e) =>
                      setCreateForm((p) => ({ ...p, [key]: Number(e.target.value) }))
                    }
                    className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-4 py-2.5 text-sm"
                  />
                </div>
      ...
      {tab === "movimientos" && (
        <div className="space-y-6">
          <div className="rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 p-6 space-y-4">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Consulta todas las entradas, salidas y transferencias.
            </p>
            <div className="flex gap-3 flex-wrap">
              <select
                value={movIdPresentacion}
                onChange={(e) => setMovIdPresentacion(e.target.value)}
                  ...
                  <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800 bg-white dark:bg-zinc-900">
                    {movimientos.map((m) => (
                      <tr key={m.idMovimiento} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition">
                        ...
                        <td className="px-5 py-3">
                          <span
                            className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                              BADGE[m.tipoMovimiento] ?? "bg-zinc-100 text-zinc-600"
                            }`}
                          >
                            {m.tipoMovimiento}
                          </span>
                        ...
                        <td className="px-5 py-3 font-medium text-zinc-900 dark:text-white">
                          {m.nombrePerfume} {m.volumenMl}ml
                        </td>
                        ...
                        <td className="px-5 py-3 text-zinc-400 text-xs whitespace-nowrap">
                          {new Date(m.fechaMovimiento).toLocaleString("es-CL")}
                        </td>
                      </tr>
                    ))}
                  </tbody>
