...
export default function TiendaPage() {
  ...
  const procesarCompra = async () => {
    ...
    try {
      ...
      if (!resPedido.ok) throw new Error("Error procesando el pedido en el backend.");

      const pedidoCreado = await resPedido.json();
      setMensaje(`Transacción exitosa. Pedido #${pedidoCreado.idPedido} confirmado.`);
      setCarrito([]); 
      setIsCartOpen(false); // Cierra el carrito tras el éxito
  ...
  return (
    <div className="animate-in fade-in duration-500 relative">
      ...
      {/* Grilla de Productos a pantalla completa */}
          ...
          <div key={p.idProducto} className="bg-white p-6 rounded-3xl shadow-sm border border-stone-200 flex flex-col justify-between hover:shadow-md transition-shadow group">
            <div>
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest bg-stone-100 px-2 py-1 rounded-md">{p.sku}</span>
            ...
            <button 
              ...
              className="mt-6 w-full bg-white text-zinc-900 border border-zinc-200 py-2.5 rounded-xl text-sm font-semibold hover:bg-zinc-50 transition-colors shadow-sm"
            >
              Agregar
            </button>
          </div>
      ...
      {/* OVERLAY Y MODAL DEL CARRITO (SIDE DRAWER) */}
      {isCartOpen && (
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                <div className="mb-6">
                  ...
                  {clientes.length === 0 ? (
                    ...
                    >
                      {clientes.map(c => (
                        <option key={c.idCliente} value={c.idCliente}>{c.nombre} {c.apellidoPaterno}</option>
                      ))}
                    </select>
                  )}
                </div>
